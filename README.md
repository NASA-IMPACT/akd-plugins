...
| [code-search-assistant](plugins/code-search-assistant/README.md) | Discover and comparatively rank public scientific code repositories for a research or technical task across NASA SMD domains — read-only, human-in-the-loop, with reliability signals; returns up to 6 grounded candidates or abstains. | `/plugin install code-search-assistant@akd-agents` | Invoke the skill directly (`/code-search-assistant:code-search`) or just ask what code/library exists for a scientific method or paper. See the [plugin README](plugins/code-search-assistant/README.md) for prerequisites and example prompts. |
